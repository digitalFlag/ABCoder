...
namespace ABCoder.ViewModels
{
    internal partial class MainWindowViewModel : ViewModel
    {
        ...
        private void SetGoley2312AE3Options()
        {
            if (string.IsNullOrEmpty(CodeInstanceGoley2312AE3.Name))
            {
                LableValueCodeInstanceName = ComboBoxCodeTypeGoley2312AE3Content;
            }
            else
            {
                LableValueCodeInstanceName = CodeInstanceGoley2312AE3.Name;

            }
            if (string.IsNullOrEmpty(CodeInstanceGoley2312AE3.Polynomial))
            {
                LableValueCodeInstancePolynomial = "x\u00b9\u00b9+x\u00b9\u2070+x\u2076+x\u2075+x\u2074+x\u00B2+1";
            }
            else
            {
                LableValueCodeInstancePolynomial = CodeInstanceGoley2312AE3.Polynomial;
            }
            LableValueCodeWordLength = CodeInstanceGoley2312AE3.CodeWordLength;
            LableValueInformationPartLength = CodeInstanceGoley2312AE3.InformationPartLength;
            LableValueVerificationPartLength = CodeInstanceGoley2312AE3.VerificationPartLength;
            LableValueErrorDetectionCapability = CodeInstanceGoley2312AE3.ErrorDetectionCapability;
            LableValueErrorCorrectionCapability = CodeInstanceGoley2312AE3.ErrorCorrectionCapability;
        }

        private void CheckGoley2312AE3InformationBitsCombination(ref StringBuilder informationInformationBitsToolTip)
        {
            if (!Goley_2312_AE3.Checks.InformationPartLength(ref _TextBoxInformationBitsText))//Lengtg Check
            {
        ...
        private void CheckGoley2312AE3CodeWord(ref StringBuilder codeCombinationToolTip)
        {
            if (!Goley_2312_AE3.Checks.CodeWordLength(ref _TextBoxCodeCombinationText))//Lengtg Check
            {
        ...
        private void EncodeGoley2312AE3()
        {
            ...
            bool[] codeCombination = Goley_2312_AE3.Code.Encode(ref informationBits);
            TextBoxCodeCombinationText = Converter.BoolArrayToBinaryString.Convert(ref codeCombination);
        }

        private void DecodeGoley2312AE3()
        {
            bool[] codeWord = Converter.BinaryStringToBoolArray.Convert(ref _TextBoxCodeCombinationText);
            bool[] errors = new bool[23];
            if (Goley_2312_AE3.Decode.IsValidCombination(ref codeWord))// No errors
            {
                bool[] infornationPart = Goley_2312_AE3.Parts.SelectTheInformationPartFromTheCodeWord(ref codeWord);
                TextBoxInformationBitsText = Converter.BoolArrayToBinaryString.Convert(ref infornationPart);
                CreateErrorBitsTextGoley2312AE3(ref codeWord, ref errors);
                return;
            }// No errors
            bool[] syndrom = Goley_2312_AE3.Decode.CalculateSindrom(ref codeWord);
            if (Goley_2312_AE3.SingleError.Check(ref syndrom, ref errors))// Single error
            {
                CreateErrorBitsTextGoley2312AE3(ref codeWord, ref errors);
                return;
            }// Single error
            if (Goley_2312_AE3.DoubleError.Check(ref syndrom, ref errors))// Double error
            {
                CreateErrorBitsTextGoley2312AE3(ref codeWord, ref errors);
                return;
            }// Double error
            if (Goley_2312_AE3.TripleError.Check(ref syndrom, ref errors))// Triple error
            {
                CreateErrorBitsTextGoley2312AE3(ref codeWord, ref errors);
                return;
            }// Triple error
        ...
        private void CreateErrorBitsTextGoley2312AE3(ref bool[] codeWord, ref bool[] errors)
        {
