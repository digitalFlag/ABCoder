...
namespace ABCoder.ViewModels
{
    ...
    {
        private void SetHamming151113Options()
        {
            if (string.IsNullOrEmpty(CodeInstanceHamming151113.Name))
            {
                LableValueCodeInstanceName = ComboBoxCodeTypeHamming151113Content;
            }
            else
            {
                LableValueCodeInstanceName = CodeInstanceHamming151113.Name;

            }
            if (string.IsNullOrEmpty(CodeInstanceHamming151113.Polynomial))
            {
                LableValueCodeInstancePolynomial = "x\u2074+x+1";
            }
            else
            {
                LableValueCodeInstancePolynomial = CodeInstanceHamming151113.Polynomial;
            }
            LableValueCodeWordLength = CodeInstanceHamming151113.CodeWordLength;
            LableValueInformationPartLength = CodeInstanceHamming151113.InformationPartLength;
            LableValueVerificationPartLength = CodeInstanceHamming151113.VerificationPartLength;
            LableValueErrorDetectionCapability = CodeInstanceHamming151113.ErrorDetectionCapability;
            LableValueErrorCorrectionCapability = CodeInstanceHamming151113.ErrorCorrectionCapability;
        }

        private void CheckHamming151113InformationBitsCombination(ref StringBuilder informationInformationBitsToolTip)
        {
            if (!Hamming_1511_13.Checks.InformationPartLength(ref _TextBoxInformationBitsText))//Lengtg Check
            {
        ...
        private void CheckHamming151113CodeWord(ref StringBuilder codeCombinationToolTip)
        {
            if (!Hamming_1511_13.Checks.CodeWordLength(ref _TextBoxCodeCombinationText))//Lengtg Check
            {
                TextBoxCodeCombinationBorderBrush = "Firebrick";
                ButtonInformationCodeCombinationTextColor = "Firebrick";
                codeCombinationToolTip.AppendLine("* The \"" + LableValueCodeCombination + "\" field length must be 15.");
        ...
        private void EncodeHamming151113()
        {
            ...
            bool[] codeCombination = Hamming_1511_13.Code.Encode(ref informationBits);
            TextBoxCodeCombinationText = Converter.BoolArrayToBinaryString.Convert(ref codeCombination);
        }

        private void DecodeHamming151113()
        {
            bool[] codeWord = Converter.BinaryStringToBoolArray.Convert(ref _TextBoxCodeCombinationText);
            bool[] errors = new bool[15];
            if (Hamming_1511_13.Decode.IsValidCombination(ref codeWord))// No errors
            {
                bool[] infornationPart = Hamming_1511_13.Parts.SelectTheInformationPartFromTheCodeWord(ref codeWord);
                TextBoxInformationBitsText = Converter.BoolArrayToBinaryString.Convert(ref infornationPart);
                CreateErrorBitsTextHamming151113(ref codeWord, ref errors);
                return;
            }// No errors
            bool[] syndrom = Hamming_1511_13.Decode.CalculateSindrom(ref codeWord);
            if (Hamming_1511_13.SingleError.Check(ref syndrom, ref errors))// Single error
            {
                CreateErrorBitsTextHamming151113(ref codeWord, ref errors);
                return;
            }// Single error
        ...
        private void CreateErrorBitsTextHamming151113(ref bool[] codeWord, ref bool[] errors)
        {
